...
| `VocabularyService`        | `home/vocabulary/`        | Named list management; vocabulary item add/remove/update with optimistic UI; `addEntry()` / `updateBack()` for single entries, `importEntries(targetListId)` for bulk import (paste or a public list) into the active deck; cross-device current-list sync via `UserPreferences` API; calls `StudyService.refreshStats()` after every add/remove. Each listed item is enriched server-side with its SRS schedule (`dueDate`/`isNew`, joined from `SrsService.getScheduleByList()`), shown in the list as a next-review label (`new` / `due` / `in 4d` / `2w` / `3mo`) in place of the old saved-age timestamp |
...
| Native (gloss) language    | Per-deployment — `langConfig.nativeLang` (`nl` Teeuw / `en` Stevens) |

### Native (gloss) language is a build-time deployment config

The target language is fixed, but the native/gloss language differs per deployment (`nl` for the Teeuw dictionary, `en` for Stevens). The browser cannot read a `.env` at runtime, so `nativeLang` is **baked into the bundle at build time**: `scripts/generate-deployment.mjs` runs before `dev`/`build`/`watch` (and after `install`) and writes `src/environments/deployment.ts` (gitignored). It resolves `NATIVE_LANG` from `process.env` first, then from the deployment's `apps/api/.env`, defaulting to `nl`. `app.constants.ts` reads `deployment.nativeLang` into `langConfig`. Changing the deployment language therefore requires a **web rebuild**, not just a server restart.
