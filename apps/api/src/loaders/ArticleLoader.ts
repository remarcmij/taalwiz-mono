...
class ArticleLoader extends BaseLoader<IArticle> {
  protected async parseContent(
    ...
    // @ts-expect-error needed because the front-matter package does not have correct typescript definitions
    const { attributes, body } = fm<Partial<IFrontMatterAttributes>>(content);
    ...
    topic.sha = md5(JSON.stringify({ topic, article }));

    const { body: newBody, hashtags } = await this.extractHashtags(
      body,
      article
    );
  ...
  protected async createData(
    ...
    if (article.hashtags.length > 0) {
      this.bulkLoadHashTags(article.hashtags, topic);
    }
  }
  ...
  private async bulkLoadHashTags(
  ...
  private async extractHashtags(body: string, article: IArticle) {
    ...
    const indexTopic = indexTopics.find(
      (topic) => topic.groupName === article.groupName
    );
    if (!indexTopic) {
