...
Words saved while reading can be reviewed later as flashcards using spaced repetition (the well-established SM-2 algorithm): cards a learner finds difficult come back sooner, well-known ones are spaced further into the future, with a simple Again / Good / Easy rating. The front shows the saved word and the back its translation, either the learner's own note or the dictionary's definition, with any word on the back tappable for a quick lookup. This is what closes the loop the rest of the app opens: read a real text, look a word up, save it, and later retain it, rather than losing it once the lesson is over. A built-in practice mode lets a learner run through a list on demand without disturbing the review schedule.
