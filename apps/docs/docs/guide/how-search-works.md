...
dictionary search box, with a focus on the morphology. It is written for linguists
and curious users rather than for developers; the implementation notes live with the
code (see [For developers](#for-developers) at the end).

## The problem

Indonesian is richly affixing. A single root such as _bakar_ ("burn") surfaces in
running text as _membakar_, _dibakar_, _membakarkan_, _terbakar_, _pembakaran_, and
more. A learner reading a sentence meets the inflected form, not the root, and types
exactly what they see. The dictionary, however, is the printed Teeuw
_Indonesisch-Nederlands Woordenboek_, whose headwords are mostly roots and a selection
of common derivations. There is no entry for every possible inflected form.

So the search has a gap to bridge: from the inflected form the user typed to a
headword the dictionary actually contains.

## The core idea: generate candidates, let the dictionary judge

Taalwiz does **not** try to analyse a word down to its one "true" root. Instead it
...
2. **Common derivations**, especially the active _meN-_ form, which the dictionary
   usually does list.
3. **Bare roots and rarer variants** last, as a fallback.
...
| --- | --- | --- |
| Suffixes | `-kan`, `-i`, `-an`, `-nya`, `-ku`, `-mu`, `-lah`, `-kah` | causative, locative, nominaliser, possessive, clitics, particles |
| Simple prefixes | `di-`, `ber-`, `ter-`, `ke-`, `se-`, `per-` | passive, stative, accidental, nominaliser |
| Nasal prefixes | `meN-` (active), `peN-` (agent noun) | assimilate to the root's first sound |
| Circumfixes | `ke-...-an`, `per-...-an`, `pe-...-an` | abstract and process nouns |
| Reduplication | `anak-anak` &rarr; `anak` | plurality, iteration |

The linguistically interesting part is the **nasal prefix _meN-_** (and its noun
counterpart _peN-_). The final nasal of the prefix assimilates to the first sound of
the root, and for one class of roots that first consonant is **dropped** entirely:

| Root initial | Realisation of _meN-_ | Example | Root |
| --- | --- | --- | --- |
...
| **_p_** (dropped) | _mem-_ | _memotong_ | _potong_ |
| **_t_** (dropped) | _men-_ | _menulis_ | _tulis_ |
| **_s_** (dropped) | _meny-_ | _menyapu_ | _sapu_ |
| **_k_** (dropped) | _meng-_ | _mengukur_ | _kukur_ |

The dropped-consonant rows are why analysis is ambiguous in the _reverse_ direction.
Seeing _menulis_, you cannot tell from the surface alone whether the root began with
...
sequenceDiagram
    ...
    Note over G: strip di- to root "bakar",<br/>rebuild active (b takes mem-) = "membakar",<br/>also emit "bakar" and the non-word "mbakar"
    ...
    S-->>U: entry shown under root "bakar"
```
...
### 2. Stopping at a derivation, not the root: _kepunyaanku_
...
circumfix all the way to the root, but the _ke-...-an_ noun _kepunyaan_ is itself a Teeuw
...
sequenceDiagram
    ...
    S->>G: generate variations
    Note over G: strip the -ku clitic to "kepunyaan",<br/>then peel ke-...-an toward the root "punya"
    ...
    Note over S: stop. The root "punya" (candidate 6)<br/>is never reached
    S-->>U: entry shown for "kepunyaan"
```

The point: the search stops at the first form Teeuw indexes, which here is the
derivation _kepunyaan_, not the bare root _punya_. The generator would reach _punya_
...
_per-...-kan_ around the root _kerja_ ("work"). The generator's attempts to rebuild an
active form here produce several shapes that are not Indonesian words. Each is dutifully
looked up and rejected before a real headword, the agent noun _pekerja_ ("worker"), is
reached.
...
The point: the generator did everything right. It produced the correct root _instal_
...
way to a confident single root would report the same emptiness, only after more work.
...
canonical root. It is the natural tool for some jobs, but **dictionary lookup is not
one of them**, for two reasons:

1. Teeuw already returns the canonical root on every successful hit, so reducing the
   query to a root first adds little.
2. The generator produces not only the root but **sideways forms** a stemmer never
   would: from passive _dibakar_ it offers active _membakar_, which is far more likely
   to be a headword than the bare root. A stemmer aiming at a single root would skip
...
the query and every word in the text to a shared root key, so that a search for
