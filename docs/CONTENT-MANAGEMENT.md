...
The **main manifest is exempt** (it is a language-agnostic index), and **dictionaries are not checked** — a dictionary's `targetLang` is its *headword* language, which may legitimately differ from the deployment language (e.g. a reverse-direction dictionary), so dictionaries remain language-agnostic.
